...
| [Flow Checkpoint](../hooks/flow-checkpoint/README.md) | Saves pipeline state between phases and validates quality gates before any phase transition in the strategic automation pipeline. Prevents gap accumulation by enforcing gate criteria at session boundaries. | sessionStart, preToolUse, sessionEnd | `checkpoint.sh`<br />`hooks.json`<br />`validate-gates.sh` |
